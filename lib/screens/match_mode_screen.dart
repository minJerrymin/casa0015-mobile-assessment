    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          Text(widget.pub.name, style: const TextStyle(color: AppTheme.muted)),
          ...
          Card(
            child: Padding(
              ...
              child: Column(
                children: [
                  const Icon(Icons.graphic_eq, size: 54, color: AppTheme.pitchGreen),
                  const SizedBox(height: 12),
                  Text('$liveNoise dB', style: Theme.of(context).textTheme.displaySmall?.copyWith(fontWeight: FontWeight.w900)),
                  const SizedBox(height: 6),
                  Text(_noiseDb == 0 ? 'Typical venue noise estimate' : 'Latest match-mode sample', style: const TextStyle(color: AppTheme.muted)),
          ...
          const Text('V0.1 uses a mock sensor sampler. V0.2 will replace this with Android microphone amplitude once device permissions are configured.', style: TextStyle(color: AppTheme.muted, fontSize: 12, height: 1.35)),
        ],
      ),
    );
  }
